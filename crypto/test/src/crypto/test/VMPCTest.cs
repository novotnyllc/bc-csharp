	...
	*/
		...
		: SimpleTest
	{
		...
		private byte[] checkEngine(VmpcEngine engine)
		{
			...
			checkByte(output, 0, 0xA8);
			checkByte(output, 1, 0x24);
			checkByte(output, 2, 0x79);
			checkByte(output, 3, 0xF5);
			checkByte(output, 252, 0xB8);
			checkByte(output, 253, 0xFC);
			checkByte(output, 254, 0x66);
			checkByte(output, 255, 0xA4);
			checkByte(output, 1020, 0xE0);
			checkByte(output, 1021, 0x56);
			checkByte(output, 1022, 0x40);
			checkByte(output, 1023, 0xA5);
			checkByte(output, 102396, 0x81);
			checkByte(output, 102397, 0xCA);
			checkByte(output, 102398, 0x49);
			checkByte(output, 102399, 0x9A);
